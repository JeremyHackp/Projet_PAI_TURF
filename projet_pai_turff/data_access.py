...
def update_graphe_individuel(graph_type, filtre_widget, graphe,
    ...
    nbr = (filtre_widget.get_state() or {}).get("nbr", 20)
    ...
    with get_connection() as conn:
        ...
        if graph_type == "Performance au cours des courses":
            cur.execute(f"""
                ...
                ORDER BY
                    SUBSTR(DateReunion,5,4)||'-'||SUBSTR(DateReunion,3,2)||'-'||SUBSTR(DateReunion,1,2)
                LIMIT {nbr}
            """, (participant_name,))
...
def update_graphe_stats_groupe(tri_nom, filtre_widget, graphe, top_n=20):
